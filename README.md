...
- **Penalties** — Early cancellation: 10% of remaining escrow distributed to nodes. Proof failure: value-proportional slash with `MIN_PROOF_FAILURE_SLASH = 0.05 ETH` floor. Forced exit: additional 50% penalty. Slashed stake burned to `address(0)`.
...
| `STAKE_PER_BYTE` | 10^14 wei | Collateral per byte of capacity |
